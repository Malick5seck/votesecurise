...
//     // États de chargement et d'erreur avec w-full max-w-full
...
//     return (
//         // Conteneur principal du formulaire sécurisé avec w-full overflow-x-hidden
...
//                 {/* Ajout de break-words sur le titre et la description */}
...
//             <form onSubmit={handleSubmit} className="w-full bg-white dark:bg-carteSombre p-5 sm:p-8 rounded-b-2xl shadow-md border border-t-0 border-gray-200 dark:border-gray-700">
...
//                     {/* Le composant QuestionSondage gérera sa propre responsivité */}
...
//                         />
//                     </div>
//                     {/* Ajout de min-w-0 pour sécuriser le flex enfant (le texte de la case à cocher) */}
...
export default function ParticiperSondage() {
    ...
    useEffect(() => {
        ...
        fetchSondage();
    }, [id, isSuperAdmin]); // On supprime currentUser des dépendances pour éviter les re-rendus bizarres
    ...
    if (estExpire && !isSuperAdmin) return (
        <div className="w-full max-w-2xl mx-auto py-12 sm:py-20 px-4 text-center transition-colors duration-300 animate-fade-in overflow-hidden">
            <div className="bg-red-50 dark:bg-red-900/20 p-6 sm:p-10 rounded-2xl shadow-lg border border-red-200 dark:border-red-800 w-full">
                <div className="text-5xl sm:text-6xl mb-4 shrink-0">⏳</div>
                <h2 className="text-xl sm:text-2xl font-bold text-red-700 dark:text-red-400 mb-2 break-words">Sondage clôturé</h2>
    ...
    if (aDejaVote && !isSuperAdmin) return (
        <div className="w-full max-w-2xl mx-auto py-12 sm:py-20 px-4 text-center transition-colors duration-300 animate-fade-in overflow-hidden">
            <div className="bg-white dark:bg-carteSombre p-6 sm:p-10 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 w-full">
                <div className="text-5xl sm:text-6xl mb-4 shrink-0">🛡️</div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white mb-2 break-words">Vous avez déjà participé !</h2>
    ...
    return (
        <div className="w-full max-w-3xl mx-auto py-6 sm:py-10 px-4 transition-colors duration-300 animate-fade-in overflow-x-hidden">
            ...
            {isSuperAdmin && (
                <div className="bg-purple-100 dark:bg-purple-900/30 border border-purple-300 dark:border-purple-800 text-purple-800 dark:text-purple-300 p-4 rounded-xl mb-6 flex items-start gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 shrink-0 mt-0.5"><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                    <div>
                        <p className="font-bold">Mode Consultation</p>
                        <p className="text-sm">En tant que Super Admin, vous consultez ce sondage en mode lecture seule. Les options de vote sont désactivées.</p>
                    </div>
                </div>
            )}
            ...
            {/* SEULS LES QUESTIONS SONT AFFICHÉES. TOUT LE RESTE (BOUTON ET CHECKBOX) EST HORS DU DOM POUR LE SUPER ADMIN */}
            <form onSubmit={handleSubmit} className={`w-full bg-white dark:bg-carteSombre p-5 sm:p-8 rounded-b-2xl shadow-md border border-t-0 border-gray-200 dark:border-gray-700 ${isSuperAdmin ? 'opacity-70 pointer-events-none' : ''}`}>
                
                {erreurSubmit && !isSuperAdmin && (
                    <div className="mb-6 p-4 bg-red-50 border-l-4 border-red-500 text-red-700 rounded text-sm sm:text-base break-words w-full">
                        <p className="font-bold">Erreur</p>
                        <p>{erreurSubmit}</p>
                    </div>
                ...
                {/* 👇 LA MAGIE OPÈRE ICI : Si l'utilisateur est un SUPER ADMIN, on ne génère RIEN de ce qui est en dessous. 👇 */}
                {isSuperAdmin === false && (
                    <>
                        <div className={`w-full mt-8 sm:mt-10 p-4 sm:p-5 rounded-xl border flex items-start gap-3 sm:gap-4 ${sondage.est_anonyme ? 'bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700' : 'bg-blue-50 border-blue-100 dark:bg-blue-900/20 dark:border-blue-800'}`}>
                            <div className="pt-0.5 shrink-0">
