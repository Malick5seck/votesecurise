...
export default function UserView({
    ...
    if (!donneesChargees) {
        return (
            <div className="flex flex-col items-center justify-center py-20 sm:py-32 bg-white dark:bg-carteSombre rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 mx-4 sm:mx-0">
                <div className="w-10 h-10 sm:w-12 sm:h-12 border-4 border-blue-200 border-t-[#3b82f6] rounded-full animate-spin mb-4"></div>
                <p className="text-gray-500 dark:text-gray-400 font-medium text-base sm:text-lg">Chargement de vos données...</p>
            </div>
        );
    }
    ...
    if (vueActuelle === 'mes-sondages') {
        ...
        return (
            <div className="bg-white dark:bg-carteSombre p-4 sm:p-6 lg:p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4 border-b border-gray-100 dark:border-gray-800 pb-5 sm:pb-6">
                    <h3 className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-white flex flex-wrap items-center gap-2 sm:gap-3">
                        <span className="shrink-0">📋 Mes sondages</span>
                        {mesSondages.length > 0 && (
                            <span className="text-xs sm:text-sm font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-400 px-2.5 sm:px-3 py-1 rounded-full">
                                {mesSondages.length}
                            </span>
                        )}
                    </h3>
                    <Link to="/creer" className="w-full sm:w-auto justify-center bg-[#3b82f6] hover:bg-blue-700 text-white font-bold py-3 sm:py-2.5 px-6 rounded-xl shadow-md shadow-blue-500/20 flex items-center gap-2 transition-transform transform hover:-translate-y-0.5">
                    ...
                    <div>
                        <div className="space-y-4">
                            {sondagesAffiches.map((sondage) => (
                                <div 
                                    key={sondage.id} 
                                    className="group flex flex-col lg:flex-row justify-between lg:items-center bg-gray-50/50 dark:bg-fondSombre p-4 sm:p-5 rounded-xl border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all hover:border-blue-200 dark:hover:border-blue-800 gap-4"
                                >
                                    <div className="w-full lg:w-1/2">
                                        <h5 className="font-extrabold text-base sm:text-lg text-gray-900 dark:text-white flex items-start sm:items-center flex-wrap gap-2 leading-tight">
                                            {sondage.titre}
                                    ...
                                    <div className="flex flex-wrap gap-2 w-full lg:w-auto justify-start sm:justify-end mt-2 lg:mt-0">
                                        <button 
                                        ...
                                        <button 
                                            ...
                                            className="w-full sm:w-auto justify-center flex items-center gap-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 font-bold py-2 px-3 sm:px-4 rounded-lg shadow-sm text-xs sm:text-sm hover:shadow-md transition-colors"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 shrink-0">
        ...
        return (
            <div>
                <div className="mb-6 sm:mb-8 text-center sm:text-left">
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white">Paramètres du compte</h2>
                </div>
                
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
                    <div className="lg:col-span-5 space-y-6 sm:space-y-8">
                        <div className="bg-white dark:bg-carteSombre p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                            <h3 className="text-lg sm:text-xl font-extrabold dark:text-white mb-5 sm:mb-6 flex items-center gap-2">👤 Informations Personnelles</h3>
                            <form onSubmit={handleUpdateProfile} className="space-y-4 sm:space-y-5">
                                ...
                                <button type="submit" disabled={loadingProfil} className="w-full bg-[#3b82f6] hover:bg-blue-700 text-white font-bold py-3.5 sm:py-4 rounded-xl shadow-lg shadow-blue-500/30 transition-colors disabled:opacity-50">Enregistrer les modifications</button>
                            </form>
                        </div>
                        
                        <div className="bg-white dark:bg-carteSombre p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                            <h3 className="text-lg sm:text-xl font-extrabold dark:text-white mb-5 sm:mb-6 flex items-center gap-2">🔒 Sécurité du compte</h3>
                            <form onSubmit={handleUpdatePassword} className="space-y-4 sm:space-y-5">
                                ...
                                <button type="submit" disabled={loadingProfil} className="w-full bg-gray-900 dark:bg-gray-700 text-white font-bold py-3.5 sm:py-4 rounded-xl hover:bg-gray-800 shadow-lg transition-colors disabled:opacity-50">Modifier le mot de passe</button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-7 space-y-6 sm:space-y-8">
                        {mesSondages.length > 0 && (
                            <div className="bg-white dark:bg-carteSombre p-6 sm:p-8 rounded-2xl shadow-sm border border-blue-100 dark:border-blue-900/30">
                                <h3 className="text-lg sm:text-xl font-extrabold dark:text-white mb-5 sm:mb-6">📊 Impact Organisateur</h3>
                                <div className="grid grid-cols-1 min-[400px]:grid-cols-3 gap-4 sm:gap-6 text-center">
                                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 sm:p-5 rounded-2xl border border-blue-100 dark:border-blue-800/30 flex min-[400px]:flex-col justify-between min-[400px]:justify-center items-center">
                                        <p className="text-[10px] sm:text-xs text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wider mb-0 min-[400px]:mb-2">Sondages Créés</p>
                                        <p className="text-2xl sm:text-4xl font-extrabold text-[#3b82f6]">{mesSondages.length}</p>
                                    </div>
                                    <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 sm:p-5 rounded-2xl border border-emerald-100 dark:border-emerald-800/30 flex min-[400px]:flex-col justify-between min-[400px]:justify-center items-center">
                                        <p className="text-[10px] sm:text-xs text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider mb-0 min-[400px]:mb-2">Votes Reçus</p>
                                        <p className="text-2xl sm:text-4xl font-extrabold text-emerald-500">{totalVotesRecus}</p>
                                    </div>
                                    <div className="bg-purple-50 dark:bg-purple-900/20 p-4 sm:p-5 rounded-2xl border border-purple-100 dark:border-purple-800/30 flex min-[400px]:flex-col justify-between min-[400px]:justify-center items-center">
                                        <p className="text-[10px] sm:text-xs text-purple-600 dark:text-purple-400 font-bold uppercase tracking-wider mb-0 min-[400px]:mb-2">Taux Moyen</p>
                                        <p className="text-xl sm:text-2xl font-extrabold text-purple-500 min-[400px]:mt-2">
                        ...
                        <div className="bg-white dark:bg-carteSombre p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                            <h3 className="text-lg sm:text-xl font-extrabold dark:text-white mb-5 sm:mb-6">🕒 Mes participations (Historique)</h3>
                            <div className="space-y-3 sm:space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                {historiqueVotes.length === 0 ? (
                                    <div className="bg-gray-50 dark:bg-fondSombre rounded-xl border border-dashed border-gray-200 dark:border-gray-700 py-10 text-center">
                                        <p className="text-gray-500 italic text-sm sm:text-base">Vous n'avez participé à aucun sondage public.</p>
                                    </div>
                                ) : (
                                    historiqueVotes.map(vote => {
                                        ...
                                        return (
                                            <div key={key} className="group bg-gray-50/50 dark:bg-fondSombre p-4 sm:p-5 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between sm:items-center gap-3 sm:gap-4 hover:shadow-md hover:border-blue-200 dark:hover:border-blue-800 transition-all">
                                                <div>
                                                    <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base line-clamp-1">{vote.sondage ? vote.sondage.titre : <span className="text-red-500 italic font-normal">Sondage supprimé par l'auteur</span>}</p>
                                                    <p className="text-xs sm:text-sm text-gray-500 mt-1 sm:mt-1.5 flex items-center gap-1.5">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 sm:w-4 sm:h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                        Voté le {new Date(vote.created_at).toLocaleDateString()} à {new Date(vote.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                                    </p>
                                                </div>
                                                ...
                                                {vote.sondage && (
                                                    <Link to={`/sondage/${vote.sondage.id}/resultats`} className="inline-flex justify-center items-center gap-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-[#3b82f6] dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-700 font-bold text-xs sm:text-sm py-2 px-4 rounded-lg transition-colors shrink-0 shadow-sm w-full sm:w-auto mt-2 sm:mt-0">
                                                        Voir les résultats
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-3.5 h-3.5 sm:w-4 sm:h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12h15m0 0l-6.75-6.75M19.5 12l-6.75 6.75" /></svg>
                                                    </Link>
                                                )}
                                            </div>
                                        );
