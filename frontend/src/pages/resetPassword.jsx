...
export default function ResetPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccesReset(true); // Déclenche la vue de succès dans la même carte
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-fondSombre py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
            <div className="max-w-md w-full bg-white dark:bg-carteSombre p-8 md:p-10 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 animate-fade-in relative overflow-hidden">
                
                {/* EN-TÊTE DYNAMIQUE (Change selon l'état de succès) */}
                <div className="text-center mb-8">
                    {succesReset ? (
                        <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-green-50 dark:bg-green-900/30 text-green-500 dark:text-green-400 mb-6 animate-fade-in">
                    ...
                    <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight transition-all">
                        {succesReset ? "Mot de passe modifié !" : "Nouveau mot de passe"}
                    </h2>
                    <p className="mt-3 text-sm text-gray-500 dark:text-gray-400 leading-relaxed transition-all">
                        {succesReset 
                ...
                {/* CONTENU DYNAMIQUE (Bouton de connexion OU Formulaire) */}
                {succesReset ? (
                    <div className="pt-4 animate-fade-in">
                        <Link to="/login" className="w-full inline-flex items-center justify-center gap-2 bg-[#3b82f6] hover:bg-blue-700 text-white font-bold py-4 px-4 rounded-xl shadow-lg shadow-blue-500/30 transition-all transform hover:-translate-y-0.5 text-lg">
                    ...
                    <div className="animate-fade-in">
                        {erreur && (
                            <div className="flex items-center gap-3 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 text-red-700 dark:text-red-400 p-4 mb-8 rounded-r-lg text-sm font-medium">
