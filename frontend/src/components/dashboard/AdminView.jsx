...
export default function AdminView({
  ...
  const safeUsers = Array.isArray(tousLesUtilisateurs)
    ? tousLesUtilisateurs
    : [];
  const safeLogs = Array.isArray(adminLogs) ? adminLogs : [];

  const totalVotes = safeSondages.reduce(
    (sum, s) => sum + (s.votes_count || 0),
    0,
  );
  const sActifs = safeSondages.filter(
    (s) => !s.date_fin || new Date(s.date_fin) > new Date(),
  ).length;
  const sExpires = safeSondages.length - sActifs;
  const topS = [...safeSondages]
    .sort((a, b) => (b.votes_count || 0) - (a.votes_count || 0))
    .slice(0, 5);

  const utilisateursFiltres = safeUsers
    .filter(
      (u) =>
        (u?.name || "")
          .toLowerCase()
    ...
    .sort((a, b) => {
      const nbSondagesA = safeSondages.filter(
        (s) => String(s.user_id) === String(a.id),
      ).length;
      const nbSondagesB = safeSondages.filter(
        (s) => String(s.user_id) === String(b.id),
      ).length;
  ...
  const sondagesFiltres = safeSondages
    .filter((s) =>
  ...
  const utilisateursPaginees = utilisateursFiltres.slice(
    indexPremierUser,
    indexDernierUser,
  );
  ...
  const sondagesPagines = sondagesFiltres.slice(
    indexPremierSondage,
    indexDernierSondage,
  );
    ...
    return (
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-fondSombre">
        <button
          ...
          className="w-full sm:w-auto px-4 py-2.5 sm:py-2 rounded-lg font-bold text-sm bg-white dark:bg-carteSombre border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors shadow-sm"
        >
          Précédent
        </button>
  ...
  return (
    <div className="w-full max-w-full overflow-hidden">
      <div className="mb-6 sm:mb-8 animate-fade-in">
        <h1 className="text-2xl sm:text-3xl font-extrabold dark:text-white truncate">
          {" "}
          Centre de Contrôle
        </h1>
      </div>
      ...
      <div className="flex overflow-x-auto gap-2 mb-6 sm:mb-8 bg-gray-100/50 dark:bg-gray-800/30 p-2 rounded-xl custom-scrollbar animate-fade-in">
        {["dashboard", "utilisateurs", "sondages", "historique", "profil"].map(
          (onglet) => (
            <button
              key={onglet}
      ...
      {!donneesChargees ? (
        ...
        <div
          className={`transition-all duration-700 ease-out transform w-full ${pageChargee ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`}
        >
          {/* ONGLET DASHBOARD */}
          {adminOngletActif === "dashboard" && (
            <div className="space-y-6 sm:space-y-8 w-full">
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                  <div className="min-w-0 mr-2">
                    <p className="text-xs sm:text-sm font-bold uppercase text-gray-500 mb-1 truncate">
                      Utilisateurs
                    </p>
                  ...
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-full text-[#3b82f6] dark:text-blue-400 shrink-0">
                    <svg
                ...
                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                  ...
                  <div className="p-3 bg-emerald-50 dark:bg-emerald-900/30 rounded-full text-emerald-600 dark:text-emerald-400 shrink-0">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                ...
                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                  ...
                  <div className="p-3 bg-purple-50 dark:bg-purple-900/30 rounded-full text-purple-600 dark:text-purple-400 shrink-0">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                ...
                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                  <div className="min-w-0 mr-2">
                    ...
                    <p className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white mt-1 truncate">
                      {sActifs}{" "}
                      <span className="text-gray-400 text-xl sm:text-2xl mx-1 font-medium">
                        /
                      </span>{" "}
                      {sExpires}
                    </p>
                  </div>
                  <div className="p-3 bg-gray-100 dark:bg-gray-800 rounded-full text-gray-600 dark:text-gray-400 shrink-0">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
              ...
              <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 dark:text-white truncate">
                  🏆 Sondages les plus populaires
                </h3>
                <div className="space-y-3">
                  {topS.map((s, index) => {
                    const auteur =
                      safeUsers.find((u) => u.id === s.user_id)?.name ||
                      "Inconnu";
                    return (
                      <div
                        key={s.id}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-gray-50 dark:bg-fondSombre rounded-xl border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow gap-3 w-full"
                      >
                        <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                          <span
                            className={`text-xl sm:text-2xl font-bold shrink-0 ${index === 0 ? "text-yellow-500" : index === 1 ? "text-gray-400" : "text-gray-300 dark:text-gray-600"}`}
                          >
                            #{index + 1}
                          </span>
                          <div className="min-w-0 flex-1">
                            <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base line-clamp-1 break-words">
                              {s.titre}
                            </p>
          ...
          {adminOngletActif === "utilisateurs" && (
            <div className="bg-white dark:bg-carteSombre rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden w-full">
              <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
                <h3 className="text-lg sm:text-xl font-bold dark:text-white truncate w-full sm:w-auto">
              ...
              <div className="overflow-x-auto w-full">
                <table className="w-full text-left min-w-[650px]">
                  <thead className="bg-gray-50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 text-xs uppercase whitespace-nowrap">
                    <tr>
                      <th className="p-3 sm:p-4">Utilisateur</th>
                      <th className="p-3 sm:p-4">Inscription</th>
                      <th className="p-3 sm:p-4">Rôle</th>
                      <th className="p-3 sm:p-4 text-center">Sondages</th>
                      <th className="p-3 sm:p-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {utilisateursPaginees.map((u) => {
                      const nbSondagesUtilisateur = safeSondages.filter(
                        (s) => s.user_id === u.id,
                      ).length;
                      ...
                      return (
                        <tr
                          key={u.id}
                          className={`hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors ${susp ? "bg-red-50/30 dark:bg-red-900/10" : ""}`}
                        >
                          <td className="p-3 sm:p-4">
                            <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base break-words line-clamp-1">
                              {u.name}
                            </p>
                          ...
                          <td className="p-3 sm:p-4 text-xs sm:text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                            {new Date(u.created_at).toLocaleDateString()}
                          </td>
                          <td className="p-3 sm:p-4 whitespace-nowrap">
                            <span
                              className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${u.role === "super_admin" ? "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400" : "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300"}`}
                            >
                              {u.role}
                            </span>
                          </td>
                            ...
                            {u.role !== "super_admin" && (
                              <>
                                <Link
                                  ...
                                  className="text-[#3b82f6] hover:text-blue-800 font-bold text-xs sm:text-sm bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 px-2 sm:px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 shrink-0"
                                ...
                                <button
                                  onClick={() => setUtilisateurASupprimer(u.id)}
                                  className="text-red-500 hover:text-red-700 font-bold text-xs sm:text-sm bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 px-2 sm:px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 shrink-0"
                ...
                {utilisateursFiltres.length === 0 && (
                  <div className="p-8 text-center text-gray-500 dark:text-gray-400 italic">
                    Aucun utilisateur ne correspond à "{rechercheUtilisateur}"
                  </div>
                )}
                {renderPagination(
                  pageUtilisateurs,
            ...
            <div className="bg-white dark:bg-carteSombre rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden w-full">
              <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 mt-1">
                    Vérifiez les questions et les résultats.
                  </p>
                </div>
                <div className="relative w-full sm:w-72 shrink-0">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <div className="overflow-x-auto w-full">
                <table className="w-full text-left min-w-[700px]">
                  <thead className="bg-gray-50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 text-xs uppercase whitespace-nowrap">
                    <tr>
                      <th className="p-3 sm:p-4">Sondage & Auteur</th>
                      <th className="p-3 sm:p-4">Statut</th>
                      <th className="p-3 sm:p-4">Votes</th>
                      <th className="p-3 sm:p-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {sondagesPagines.map((s) => {
                      const estExpire =
                        s.date_fin && new Date(s.date_fin) < new Date();
                      const auteur =
                        safeUsers.find((u) => u.id === s.user_id)?.name ||
                        "Inconnu";
                      return (
                        ...
                        >
                          <td className="p-3 sm:p-4">
                            ...
                            <p className="text-xs text-gray-500 mt-1 line-clamp-1 break-words">
                              ...
                              • {s.est_anonyme ? "Anonyme" : "Public"}
                            </p>
                          </td>
                          <td className="p-3 sm:p-4 whitespace-nowrap">
                            <span
                              className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${estExpire ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                            >
                              {estExpire ? "Expiré" : "Actif"}
                            </span>
                          </td>
                          <td className="p-3 sm:p-4 font-bold text-gray-700 dark:text-gray-300 text-sm sm:text-base whitespace-nowrap">
                            {s.votes_count || 0}
                          </td>
                          <td className="p-3 sm:p-4 flex justify-end gap-2 whitespace-nowrap">
                            <Link
                              to={`/sondage/${s.id}`}
                              className="text-emerald-600 hover:text-emerald-800 font-bold text-xs sm:text-sm bg-emerald-50 dark:bg-emerald-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1 shrink-0"
                              title="Aperçu"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                            ...
                            <Link
                              to={`/sondage/${s.id}/resultats`}
                              className="text-blue-600 hover:text-blue-800 font-bold text-xs sm:text-sm bg-blue-50 dark:bg-blue-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1 shrink-0"
                              title="Résultats"
                            >
                              <span>📊</span>{" "}
                            ...
                            {!estExpire && (
                              <button
                                onClick={() => setSondageACloturer(s.id)}
                                className="text-orange-600 hover:text-orange-800 font-bold text-xs sm:text-sm bg-orange-50 dark:bg-orange-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1 shrink-0"
                              >
                                <span>🔒</span>{" "}
                                <span className="hidden sm:inline">
                                  Clôturer
                            ...
                            <button
                              onClick={() => setSondageAdminASupprimer(s.id)}
                              className="text-red-500 hover:text-red-700 font-bold text-xs sm:text-sm bg-red-50 dark:bg-red-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1 shrink-0"
                            >
                              <span>🗑️</span>{" "}
                              <span className="hidden sm:inline">
                                Supprimer
                ...
                {sondagesFiltres.length === 0 && (
                  <div className="p-8 text-center text-gray-500 dark:text-gray-400 italic">
                    Aucun sondage ne correspond à "{rechercheSondage}"
                  </div>
                )}
                {renderPagination(
                  pageSondages,
            ...
            <div className="bg-white dark:bg-carteSombre p-4 sm:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 w-full overflow-hidden">
              <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 dark:text-white flex items-center gap-3 truncate">
                <svg
                  ...
                  strokeWidth={2}
                  stroke="currentColor"
                  className="w-5 h-5 sm:w-6 sm:h-6 text-purple-500 shrink-0"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              <div className="space-y-3 sm:space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar w-full">
                {safeLogs.length === 0 ? (
                  ...
                  safeLogs.map((log, index) => (
                    <div
                      key={index}
                      className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5 bg-gray-50/50 dark:bg-fondSombre p-4 sm:p-5 rounded-xl border border-gray-200 dark:border-gray-700 transition-all hover:shadow-md hover:-translate-y-0.5 w-full"
                    >
                      <div
                        className={`self-start sm:self-auto p-2 sm:p-3 rounded-xl shadow-sm shrink-0 ${log.action === "ban" ? "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400" : log.action === "cloture" ? "bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400" : "bg-gray-200 text-gray-700 dark:bg-gray-800 dark:text-gray-400"}`}
                      >
                        {log.action === "ban"
                          ? "🔨"
                          : log.action === "cloture"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base leading-snug break-words">
                          {log.description}
                        </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 w-full">
              <div className="bg-white dark:bg-carteSombre p-5 sm:p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 w-full">
                ...
                <div className="mb-6 bg-purple-50 dark:bg-purple-900/20 p-4 rounded-xl border border-purple-100 dark:border-purple-800/50">
                  <p className="text-xs sm:text-sm font-bold text-purple-700 dark:text-purple-400 uppercase break-words">
                    Rôle : {user.role.replace("_", " ")}
                  </p>
                  <p className="text-xs sm:text-sm text-purple-600/80 dark:text-purple-400/80 mt-1">
                    Privilèges maximaux accordés.
                  </p>
                </div>
                <form
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full p-3 bg-gray-50 dark:bg-fondSombre border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-[#3b82f6] outline-none dark:text-white text-sm sm:text-base"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  <button
                    ...
                    className="w-full bg-[#3b82f6] text-white font-bold py-3.5 sm:py-3 rounded-xl hover:bg-blue-700 transition-colors"
                  >
                    Mettre à jour
                  </button>
                </form>
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full p-3 bg-gray-50 dark:bg-fondSombre border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none dark:text-white text-sm sm:text-base"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  <button
                    type="submit"
                    disabled={loadingProfil}
                    className="w-full bg-gray-900 dark:bg-gray-700 text-white font-bold py-3.5 sm:py-3 rounded-xl hover:bg-gray-800 transition-colors"
                  >
