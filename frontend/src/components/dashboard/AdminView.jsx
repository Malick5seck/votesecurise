    ...
    return (
        <div>
            <div className="mb-6 sm:mb-8 animate-fade-in">
                <h1 className="text-2xl sm:text-3xl font-extrabold dark:text-white"> Centre de Contrôle</h1>
            </div>
            ...
            <div className="flex overflow-x-auto gap-2 mb-6 sm:mb-8 bg-gray-100/50 dark:bg-gray-800/30 p-2 rounded-xl custom-scrollbar animate-fade-in">
                {['dashboard', 'utilisateurs', 'sondages', 'historique', 'profil'].map(onglet => (
                    <button 
                        ...
                        className={`px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg font-bold text-sm transition-all whitespace-nowrap ${adminOngletActif === onglet ? 'bg-white dark:bg-carteSombre text-[#3b82f6] dark:text-blue-400 shadow-sm border border-gray-200 dark:border-gray-700' : 'text-gray-500 hover:bg-white/50 dark:hover:bg-gray-700/50'}`}
                    >
            ...
            {!donneesChargees ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-carteSombre rounded-2xl border border-gray-100 dark:border-gray-800 animate-fade-in">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 font-medium">Initialisation du Centre de Contrôle...</p>
                </div>
            ) : (
                <div className={`transition-all duration-700 ease-out transform ${pageChargee ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        ...
                        <div className="space-y-6 sm:space-y-8">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                                    <div><p className="text-xs sm:text-sm font-bold uppercase text-gray-500 mb-1">Utilisateurs</p><p className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white">{safeUsers.length}</p></div>
                                ...
                                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                                    <div><p className="text-xs sm:text-sm font-bold uppercase text-gray-500 mb-1">Sondages</p><p className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white">{safeSondages.length}</p></div>
                                ...
                                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                                    <div><p className="text-xs sm:text-sm font-bold uppercase text-gray-500 mb-1">Votes récoltés</p><p className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white">{totalVotes}</p></div>
                                ...
                                <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start justify-between transform transition-all hover:scale-105 duration-300">
                                    <div><p className="text-xs sm:text-sm font-bold uppercase text-gray-500 mb-1">Actifs / Expirés</p><p className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white mt-1">{sActifs} <span className="text-gray-400 text-xl sm:text-2xl mx-1 font-medium">/</span> {sExpires}</p></div>
                            ...
                            <div className="bg-white dark:bg-carteSombre p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                                <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 dark:text-white">🏆 Sondages les plus populaires</h3>
                                <div className="space-y-3">
                                    {topS.map((s, index) => {
                                        const auteur = safeUsers.find(u => u.id === s.user_id)?.name || 'Inconnu';
                                        return (
                                            <div key={s.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-gray-50 dark:bg-fondSombre rounded-xl border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow gap-3">
                                                <div className="flex items-center gap-3 sm:gap-4">
                                                    ...
                                                    <div>
                                                        <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base line-clamp-1">{s.titre}</p>
                                                        <p className="text-xs sm:text-sm text-gray-500">Par {auteur} • Créé le {new Date(s.created_at).toLocaleDateString()}</p>
                                                    </div>
                                                </div>
                                                <div className="self-start sm:self-auto shrink-0 bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 font-bold px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-xs sm:text-sm">
                    ...
                    {adminOngletActif === 'utilisateurs' && (
                        <div className="bg-white dark:bg-carteSombre rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                            <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <h3 className="text-lg sm:text-xl font-bold dark:text-white">Annuaire des Utilisateurs</h3>
                                            ...
                                            return (
                                                <tr key={u.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors ${susp ? 'bg-red-50/30 dark:bg-red-900/10' : ''}`}>
                                                    <td className="p-3 sm:p-4">
                                                        <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base">{u.name}</p>
                                                        <p className="text-xs sm:text-sm text-gray-500 truncate max-w-[150px] sm:max-w-xs">{u.email}</p>
                                                        {susp && (
                                                            //  Gère le wrap sur les petits écrans pour le badge de suspension
                                                            <div className="mt-1.5 flex flex-wrap">
                                                                <span className="inline-flex items-center rounded-full bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-200 text-[10px] font-extrabold uppercase tracking-wide px-2 py-0.5">
                                                                    {susp.type === 'permanent'
                                                                        ? 'Compte suspendu'
                                                    ...
                                                    <td className="p-3 sm:p-4 text-xs sm:text-sm text-gray-600 dark:text-gray-400">{new Date(u.created_at).toLocaleDateString()}</td>
                                                    <td className="p-3 sm:p-4"><span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${u.role === 'super_admin' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300'}`}>{u.role}</span></td>
                                                    ...
                                                    <td className="p-3 sm:p-4 flex justify-end gap-2 whitespace-nowrap">
                                                        {u.role !== 'super_admin' && (
                                                            <>
                                                                <Link to={`/admin/utilisateurs/${u.id}`} className="text-[#3b82f6] hover:text-blue-800 font-bold text-xs sm:text-sm bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 px-2 sm:px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5" title="Voir l'historique">
                                                                ...
                                                                {/* 🔥 Bouton renommé "Suspendre" */}
                                                                <button onClick={() => setUtilisateurASupprimer(u.id)} className="text-red-500 hover:text-red-700 font-bold text-xs sm:text-sm bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 px-2 sm:px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5" title="Suspendre l'utilisateur">
                    ...
                    {adminOngletActif === 'sondages' && (
                        <div className="bg-white dark:bg-carteSombre rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                            <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <div><h3 className="text-lg sm:text-xl font-bold dark:text-white">Modération des contenus</h3><p className="text-xs sm:text-sm text-gray-500 mt-1">Vérifiez les questions et les résultats.</p></div>
                                <div className="relative w-full sm:w-72">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></div>
                                    <input 
                                        type="text" 
                                        ...
                                        {sondagesPagines.map(s => {
                                            ...
                                            return (
                                                <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                                                    <td className="p-3 sm:p-4"><p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base line-clamp-1">{s.titre}</p><p className="text-xs text-gray-500 mt-1">Par <span className="font-bold text-[#3b82f6]">{auteur}</span> • {s.est_anonyme ? 'Anonyme' : 'Public'}</p></td>
                                                    <td className="p-3 sm:p-4"><span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${estExpire ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{estExpire ? 'Expiré' : 'Actif'}</span></td>
                                                    <td className="p-3 sm:p-4 font-bold text-gray-700 dark:text-gray-300 text-sm sm:text-base">{s.votes_count || 0}</td>
                                                    <td className="p-3 sm:p-4 flex justify-end gap-2 whitespace-nowrap">
                                                        <Link to={`/sondage/${s.id}`} className="text-emerald-600 hover:text-emerald-800 font-bold text-xs sm:text-sm bg-emerald-50 dark:bg-emerald-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1" title="Aperçu">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                                            <span className="hidden sm:inline">Aperçu</span>
                                                        </Link>
                                                        <Link to={`/sondage/${s.id}/resultats`} className="text-blue-600 hover:text-blue-800 font-bold text-xs sm:text-sm bg-blue-50 dark:bg-blue-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1" title="Résultats">
                                                            <span>📊</span> <span className="hidden sm:inline">Stats</span>
                                                        </Link>
                                                        {!estExpire && (
                                                            <button onClick={() => setSondageACloturer(s.id)} className="text-orange-600 hover:text-orange-800 font-bold text-xs sm:text-sm bg-orange-50 dark:bg-orange-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1">
                                                                <span>🔒</span> <span className="hidden sm:inline">Clôturer</span>
                                                            </button>
                                                        )}
                                                        <button onClick={() => setSondageAdminASupprimer(s.id)} className="text-red-500 hover:text-red-700 font-bold text-xs sm:text-sm bg-red-50 dark:bg-red-900/20 px-2 sm:px-3 py-1.5 rounded-lg flex items-center gap-1">
                    ...
                    {adminOngletActif === 'historique' && (
                        <div className="bg-white dark:bg-carteSombre p-4 sm:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                            <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 dark:text-white flex items-center gap-3">
                            ...
                            <div className="space-y-3 sm:space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                                {safeLogs.length === 0 ? (
                                    ...
                                    safeLogs.map((log, index) => (
                                        <div key={index} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5 bg-gray-50/50 dark:bg-fondSombre p-4 sm:p-5 rounded-xl border border-gray-200 dark:border-gray-700 transition-all hover:shadow-md hover:-translate-y-0.5">
                                            ...
                                            <div className="flex-1">
                                                <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base leading-snug">{log.description}</p>
                                                <p className="text-xs sm:text-sm text-gray-500 mt-1 flex items-center gap-1.5">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3 h-3 sm:w-4 sm:h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    ...
                    {adminOngletActif === 'profil' && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
                            <div className="bg-white dark:bg-carteSombre p-5 sm:p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                                ...
                                <div className="mb-6 bg-purple-50 dark:bg-purple-900/20 p-4 rounded-xl border border-purple-100 dark:border-purple-800/50">
                                    <p className="text-xs sm:text-sm font-bold text-purple-700 dark:text-purple-400 uppercase">Rôle : {user.role.replace('_', ' ')}</p>
